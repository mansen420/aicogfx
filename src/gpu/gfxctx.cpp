...
ctx::shader_t ctx::compile(ctx::stageinfo info, opres_t* res)const noexcept
{
...
ctx::program_t ctx::link(const std::vector<shader_t>& stages, opres_t* res)const noexcept
{
    program_t prog;
    if(!_impl::hndl(prog))
    {
...
opres_t ctx::bind(program_t prog)const noexcept
{
...
opres_t ctx::bind(const vtxlayout_t& layout)const noexcept
{
...
ctx::buf_t ctx::bufalloc(bufinfo info, const void* data, opres_t* res)const noexcept
{
...
opres_t ctx::bufdata(const buf_t& buffer, const void* data, size_t size,
...
opres_t ctx::_init(const wndctx::info& info)noexcept{return implptr->init(info);}
...
opres_t ctx::_impl::init(const wndctx::info& wndinfo)noexcept
{
    if(wndinfo.flags & wndctx::bits::DEBUGCTX)
    {
        GLint GLflags;
