...
namespace aico::sys
{
    ...
    void* malc(size_t bytes, size_t alignment, opres_t* res)noexcept
    {
    ...
    size_t nr_objs(T* addr)
    {
        ...
        //TODO make this use a retval
        if(!hdr) return 0; //the fuck?
        //XXX this is fragile, relies on hdr_t too much, consider storing obj count
        //in metadata directly
        return (hdr->bytes-((uintptr_t)addr-(uintptr_t)hdr->base))/sizeof(T);
    }
}
