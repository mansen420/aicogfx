...
namespace aico::gpu
{
    ...
    public:
        ...
        [[nodiscard]]buf_t bufalloc(bufinfo, const void* =nullptr, opres_t* =nullptr)
            const noexcept;
        opres_t bufdata(const buf_t&, const void* data, size_t size, size_t buf_offset)
            const noexcept;
        ...
        opres_t bind(const vtxlayout_t&)const noexcept;
        ...
        [[nodiscard]]shader_t compile(stageinfo, opres_t* =nullptr)const noexcept;
        [[nodiscard]]program_t link(const std::vector<shader_t>&, opres_t* =nullptr)const 
            noexcept;
        //TODO this is an illusion, there is no global state, make user
        //pass in own renderpass state explicitly, vulkan-style
        opres_t bind(program_t)const noexcept;
        void free(program_t&)const noexcept;
        void free(shader_t&)const noexcept;
    private:
        gfxctx(gfxconf_t);

        opres_t _init(const gpu::wndctx::info&) noexcept;
